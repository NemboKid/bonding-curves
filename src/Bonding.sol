...
contract BondingToken is ERC20 {
    ...
    function buyTokens(uint256 amount, uint256 maxTotalPrice, uint256 deadline) public payable {
    ...
    function sellTokens(uint256 amount, uint256 minTotalPrice, uint256 deadline) public {
        if (block.timestamp > deadline) {
            revert DeadlineExceeded();
        }
    ...
    function _getTotalPriceForPurchase(uint256 amount) internal view returns (uint256 totalPrice) {
        // example: if the user buys 2 tokens when supply is 10, he should pay 1.10 + 1.11 = 2.21
        for (uint256 i = 0; i < amount;) {
            totalPrice += INITIAL_PRICE + ((totalSupply() + i) * PRICE_INCREMENT);
            unchecked {
                ++i;
            }
        }
    ...
    /// @return totalPrice The total price for the given amount of tokens.
    function _getTotalPriceForSale(uint256 amount) internal view returns (uint256 totalPrice) {
        for (uint256 i = 0; i < amount;) {
            totalPrice += INITIAL_PRICE + ((totalSupply() - i) * PRICE_INCREMENT);
            unchecked {
                ++i;
    ...
    /// @dev Receives Ether sent to the contract.
